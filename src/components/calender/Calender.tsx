        ...
        <div className='p-8 max-w-screen max-h-screen w-full h-full flex flex-col'>
            ...
            <table className='w-full flex-1'>
                <thead>
                    <tr className='bg-gray-200'>
                        {days.map((day, index) => (
                            <th
                                key={index}
                                className='w-[14.2857%] px-6 py-3 text-xs font-medium text-gray-600 uppercase tracking-wider'
                            >
                                {day}
                            </th>
                        ))}
                ...
                <tbody>
                    {[
                        ...Array(
                            Math.ceil((firstDayOfMonth + daysInMonth) / 7)
                        ).keys(),
                    ].map((weekIndex) => (
                        <tr key={weekIndex}>
                            {[...Array(7).keys()].map((dayIndex) => {
                                ...
                                return (
                                    <td
                                        key={dayIndex}
                                        className={`w-[14.2857%] px-6 py-4 text-sm border-2 text-gray-800 relative ${
                                        ...
                                        >
                                            {dayNumber > 0 &&
                                                dayNumber <= daysInMonth && (
                                                    <>
                                                        <p
                                                            className={`absolute top-2 left-2 text-xs p-2 rounded-full ${
                                                                isToday
